<template>
  <v-navigation-drawer v-model="sidebar.isOpen" location="right" temporary width="720">
    <v-toolbar flat density="comfortable">
      <v-toolbar-title>
        {{ isEdit ? 'Editar Ordem de Compra' : isView ? 'Visualizar Ordem de Compra' : 'Cadastrar Ordem de Compra' }}
      </v-toolbar-title>
      <v-spacer />
    ...
    <v-form ref="formRef" v-model="formValid" class="pa-4" @submit.prevent="submit">
      <v-row dense>
        <v-col cols="12" md="6">
          <v-text-field v-model="form.commitmentNoteNumber" label="Nº da NE" :rules="[rules.required]" hide-details="auto" />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field v-model="form.issuingBody" label="Órgão Emissor" :rules="[rules.required]" hide-details="auto" />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field v-model="form.year" label="Ano" type="number" :rules="[rules.required]" hide-details="auto" />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field v-model="form.processNumber" label="Processo" :rules="[rules.required]" hide-details="auto" />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field
          ...
          <div class="mt-1">
            ...
            <template v-else-if="orderLookupStatus === 'found'">
              <v-chip size="small" color="green" variant="tonal">Pedido encontrado: #{{ form.orderId }}</v-chip>
            </template>
        ...
        <v-col cols="12" md="6">
          <v-select
            ...
            return-object="false"
            @update:modelValue="val => form.supplierCompanyId = typeof val === 'object' && val !== null ? val.id : val"
          />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field v-model="form.totalValue" label="Valor" type="number" :rules="[rules.required]" hide-details="auto" />
        </v-col>
        <v-col cols="12" md="6">
          <v-text-field v-model="form.issueDate" label="Data de Emissão" type="date" :rules="[rules.required]" hide-details="auto" />
        </v-col>
        <v-col cols="12" md="6">
          <v-select v-model="form.status" :items="deliveryStatusOptions" label="Status da Entrega" :rules="[rules.required]" hide-details="auto" />
        </v-col>
        <v-col cols="12" md="6">
          <v-select v-model="form.emailStatus" :items="emailStatusOptions" label="Status do E-mail" :rules="[rules.required]" hide-details="auto" />
        </v-col>
        <v-col cols="12">
...
export default {
  ...
  computed: {
    ...
    supplierOptions() {
      return this.suppliers.map(s => ({ id: s.id, name: s.name || s.razaoSocial || s.nomeFantasia || `#${s.id}` }));
    },
    canSubmit() {
      return Boolean(
  ...
  let list = [];
        ...
        const found = (list || []).find(
          (o) => String(o.orderNumber || o.metadata?.orderNumber || o.metadata?.numero || o.id) === num
        );
        if (found) {
    ...
    async submit() {
      ...
      try {
        ...
        if (payload.supplierCompanyId !== undefined && payload.supplierCompanyId !== null && payload.supplierCompanyId !== '') {
        ...
        this.error = this.isEdit ? 'Falha ao atualizar ordem de compra.' : 'Falha ao salvar ordem de compra.';
      } finally {
        this.loading = false;
      }
    },
