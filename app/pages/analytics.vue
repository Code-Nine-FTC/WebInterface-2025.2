...
function drawChart() {
  ...
  // max value
  let maxVal = 0
  for (const s of series) for (const v of s.data) if (typeof v === 'number' && v > maxVal) maxVal = v
...
watch([() => analytics.demandaSeries.categories, () => analytics.demandaSeries.series, yMetric], () => {
...
<template>
  <div class="analytics-page">
    ...
    <!-- Abas -->
    <div class="tabs">
      <button @click="tab='top'" :class="['tab', tab==='top' ? 'active' : '']">Top materiais</button>
      <button @click="tab='grupos'" :class="['tab', tab==='grupos' ? 'active' : '']">Demanda por grupo</button>
    </div>
    ...
    <div v-if="tab==='top'" class="panel">
      ...
      <div v-else-if="!analytics.topMateriais.length" class="state empty">Sem dados para o período escolhido.</div>
    ...
    <div v-else class="panel">
      ...
      <div class="section">
        ...
        <div v-else-if="!analytics.demandaPorGrupo.length" class="state empty">Sem dados para o período escolhido.</div>
        <div v-else class="table-card">
          <div class="table-wrapper">
            <table class="table">
              <thead>
...
.state { color: #4b5563; font-size: 14px; }
