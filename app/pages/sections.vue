  ...
  <div class="max-w-6xl mx-auto p-6">
    <v-card class="bg-white rounded-lg shadow-md pa-4 mb-4">
      ...
      <div class="d-flex flex-wrap gap-3 mb-2">
        <v-select v-model="filterType" :items="typeOptions" label="Tipo" density="comfortable" variant="outlined" hide-details="auto" style="max-width: 220px" />
        <v-select v-model="filterActive" :items="activeOptions" label="Status" density="comfortable" variant="outlined" hide-details="auto" style="max-width: 220px" />
        <v-text-field v-model="search" label="Buscar por título" prepend-inner-icon="mdi-magnify" density="comfortable" variant="outlined" hide-details="auto" style="min-width: 260px" />
        <v-spacer />
    ...
    <v-card class="bg-white rounded-lg shadow-md pa-4">
      <div class="px-4 py-3 border-b border-slate-100">
        <div class="d-flex items-center gap-2 text-xs text-slate-500">
          ...
          <v-spacer />
          <v-chip color="green" size="x-small" variant="tonal" class="font-medium">{{ filtered.length }}</v-chip>
        </div>
      </div>
      <div class="p-2">
        <v-data-table :headers="headers" :items="filtered" item-key="id" :loading="loading" class="min-w-[900px]">
          <template #item.sectionType="{ item }">
            <v-chip size="x-small" :color="item.sectionType === 'CONSUMER' ? 'indigo' : 'blue'" variant="tonal">{{ item.sectionType }}</v-chip>
          </template>
          <template #item.isActive="{ item }">
            <v-chip size="x-small" :color="item.isActive ? 'green' : 'grey'" variant="tonal">{{ item.isActive ? 'Ativa' : 'Inativa' }}</v-chip>
          </template>
          <template #item.createdAt="{ item }">{{ formatDate(item.createdAt) }}</template>
          <template #item.lastUpdate="{ item }">{{ formatDate(item.lastUpdate || item.updatedAt) }}</template>
          <template #item.actions="{ item }">
            <div class="d-flex align-center gap-1">
              <v-tooltip text="Editar" location="top">
                <template #activator="{ props }">
                  <v-btn v-bind="props" size="small" icon="mdi-pencil" variant="text" color="primary" @click="openEdit(item)" />
                </template>
              </v-tooltip>
              <v-tooltip :text="item.isActive ? 'Inativar' : 'Ativar'" location="top">
                <template #activator="{ props }">
                  <v-btn v-bind="props" size="small" :icon="item.isActive ? 'mdi-cancel' : 'mdi-check-circle'" variant="text" :color="item.isActive ? 'red' : 'green'" @click="toggleActive(item)" />
                </template>
              </v-tooltip>
            </div>
          </template>
    ...
    <v-dialog v-model="dialog" max-width="520">
      <v-card>
        <v-card-title class="text-subtitle-1">{{ editing ? 'Editar Seção' : 'Nova Seção' }}</v-card-title>
        <v-card-text>
          ...
          <v-form ref="formRef" v-model="formValid" @submit.prevent="save">
            <v-text-field v-model="form.title" label="Título" variant="outlined" density="comfortable" :rules="[rules.required]" />
            <v-select v-model="form.sectionType" :items="typeOnlyOptions" label="Tipo" variant="outlined" density="comfortable" :rules="[rules.required]" />
            <v-text-field v-model="form.roleAccess" label="Role Access (opcional)" variant="outlined" density="comfortable" />
            <div class="d-flex align-center mt-2">
              ...
              <v-btn type="submit" color="primary" :loading="saving">{{ editing ? 'Salvar' : 'Criar' }}</v-btn>
            </div>
          </v-form>
        </v-card-text>
      </v-card>
...
const form = ref<{ id?: number; title: string; sectionType: 'CONSUMER'|'STORAGE'; isActive: boolean; roleAccess?: string | null }>({
  title: '', sectionType: 'CONSUMER', isActive: true, roleAccess: ''
})
